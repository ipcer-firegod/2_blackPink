...
### 1. 局部作用域分类；let/const 与 var 作用域差异
- 局部作用域分为：**函数作用域**、**块级作用域**（`{}` 包裹）。
- `var` 只有**函数作用域**，无块级作用域，存在变量提升；
- `let/const` 支持**块级作用域**，不允许重复声明，存在暂时性死区。

### 2. 闭包的定义、作用、问题
- 定义：**函数嵌套时，内部函数引用外部函数变量，形成闭包**。
- 作用：**延长变量生命周期**、实现私有变量、避免全局污染。
- 问题：**变量无法被垃圾回收**，易造成内存泄漏，需手动释放。

### 3. 箭头函数与普通函数核心差异；事件回调不推荐原因
- 差异：箭头函数无 `this`、无 `arguments`、不能当构造函数、不能用作生成器。
- 事件不推荐：箭头函数 `this` 继承外层，**不指向事件源**，不方便获取当前操作元素。

### 4. 剩余参数与展开运算符区别及场景
- 剩余参数 `...args`：**把多个参数合成一个数组**，用在**函数形参**。
- 展开运算符 `...arr`：**把数组展开成多个值**，用在**函数调用、数组/对象拼接**。

### 5. 数组解构数量不匹配处理
- 变量少、值多：**取前面的，多余忽略**；
- 变量多、值少：**多余变量为 undefined**，可设置默认值避免。

### 6. forEach 和 filter 作用与返回值
- `forEach`：**遍历数组**，无返回值（`undefined`）；
- `filter`：**筛选满足条件的元素**，返回**新数组**。

### 7. 对象解构重命名、多级解构写法
- 重命名：`const { name: username } = obj`；
- 多级解构：`const { user: { age } } = obj`。
