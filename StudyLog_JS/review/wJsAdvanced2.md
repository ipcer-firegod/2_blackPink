...
数组.splice(起始索引, 删除个数, 新增元素1, 新增元素2...)

​	对数组进行**删除、插入、替换**，**直接修改原数组**，并返回**被删除元素组成的新数组**。

数组.slice(起始索引, 结束索引)

​	**截取数组的一段**，返回**新数组**，**原数组不变**。	
...
- 定义：有 length 属性、按索引存储数据，但无数组方法（如 `arguments`、DOM 集合）。
  - 转换方式：`Array.from(伪数组)` 或 `[...伪数组]`。（通过 `call` 调用数组的 `slice` 方法来实现转换：Array.prototype.slice.call(arguments)，不如前面两个简洁常用了）		
...
#### 2. 适用对象不同
- **for…in**：可以遍历 **对象、数组、字符串**
- **for…of**：只能遍历 **可迭代对象**（数组、字符串、Map、Set 等），**不能直接遍历普通对象**

#### 3. 是否遍历原型属性
- **for…in**：会把 **原型上的可枚举属性** 也遍历出来
- **for…of**：只遍历 **自身成员**，不碰原型
...
### 1. 构造函数约定 & 实例化过程
- 约定：① 函数名**首字母大写**；② 必须和 `new` 一起使用。
- 过程：创建空对象 → 指向原型 → 执行构造函数赋值 → 返回实例对象。

### 2. 实例成员 vs 静态成员
- 实例成员：`this.xx`，只能**实例.xx**访问；
- 静态成员：`构造函数.xx`，只能**构造函数.xx**访问。

### 3. 三个 Object 方法作用
- `Object.keys(obj)`：返回**自身可枚举属性名**组成的数组；
- `Object.values(obj)`：返回**属性值**组成的数组；
- `Object.assign(目标, ...源)`：**浅拷贝合并对象**，返回目标对象。

### 4. reduce 起始值作用
- 作用：作为**第一次累计的初始值**，更安全、结果更可控；
- 无起始值：第一次累计用**数组第 0 项**，空数组会报错，结果易不符合预期。

### 5. 伪数组转真数组 & 伪数组特点
- 转换：`Array.from(arrLike)`、`[...arrLike]`、`Array.prototype.slice.call(arrLike)`；
- 特点：有**length**、可下标访问，但**无数组方法**（如 forEach、push）。

### 6. toFixed(2)
- 作用：数字**保留 2 位小数**并四舍五入；
- 返回值：**字符串**，不是数字。

### 7. 购物车常用方法
- 规格拼接：`map` + `join`；
- 赠品处理：`filter` 筛选、`some` 判断、`forEach` 渲染。
