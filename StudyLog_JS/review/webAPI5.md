...
- 顺序：先执行**同步任务**，异步任务放入队列，同步完成后再执行**异步任务**。

- 事件循环：先执行执行栈同步 → 执行微任务 → 渲染页面 → 执行宏任务，循环往复。

  <!--! 有关事件循环需要注意，可在豆包收藏对话找到对应内容 !--!>
...
- **localStorage**：生命周期**永久**，同源页面**数据共享**；
- **sessionStorage**：生命周期**当前窗口**，关闭即清除，**不共享**。

### 3. 本地存储复杂数据类型
- 必须用 **JSON 序列化**：
- 存：`JSON.stringify(对象/数组)`
- 取：`JSON.parse(字符串)`
...
- `search`：? 后面的参数
- `hash`：# 后面的锚点
- `reload(true)`：**强制刷新页面**（跳过缓存）

### 5. map 和 join 作用
- `map`：把数组**转换成新数组**（常用于转HTML结构）；
- `join('')`：把数组**拼成字符串**；
- 优点：批量生成DOM结构，**只插入一次**，性能高。

### 6. 学生ID自增 & 去重
- 自增：取最后一条数据的ID **+1**；
- 核心：每次新增前**判断ID是否已存在**，不存在才添加。
