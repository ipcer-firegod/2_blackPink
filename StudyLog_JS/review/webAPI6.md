...
### 1. 正则作用；test 和 exec 区别
- 核心作用：**校验数据格式、查找、替换、提取字符串**。
- `test()`：返回**布尔值**，判断是否匹配；
- `exec()`：返回**匹配数组或 null**，可提取内容。

### 2. 边界符 ^ $ 作用；遗漏问题
- `^` 匹配**开头**，`$` 匹配**结尾**；
- 遗漏会导致**包含即可通过**，无法精确校验完整字符串。

### 3. 量词 {n,m} 规则；3~8 位数字正则
- 逗号两侧**不能有空格**，否则会被当作普通字符匹配；
- 正则：`/^\d{3,8}$/`
...
### 5. 修饰符 i g；全局敏感词替换
- `i`：**忽略大小写**；`g`：**全局匹配**；
- 替换：`str.replace(/敏感词/gi, '***')`。

### 6. 大陆手机号正则；号段逻辑
- 正则：`/^1[3-9]\d{9}$/`
- 逻辑：以 **1 开头**，第二位 **3~9**，后面 **9 位数字**，共 11 位。

### 7. 注册表单提交校验流程
- 流程：点击提交 → **逐个校验表单项** → 有一项不通过则阻止提交；
- 确保全部通过：用**开关变量（flag）**，全部校验成功才为 true，允许提交。
