...
### 1. 事件流阶段+执行顺序+阻止冒泡
- 两个阶段：**捕获阶段**（从根节点到目标元素）、**冒泡阶段**（从目标元素到根节点）；
- 执行顺序：先捕获，后触发目标元素事件，最后冒泡；
- 阻止冒泡：事件处理函数中调用`e.stopPropagation()`。

### 2. 事件委托原理+好处+li点击实现
- 核心原理：利用**事件冒泡**，将子元素的事件绑定到共同父元素上，通过`e.target`判断触发的子元素；
- 好处：减少事件绑定次数、提升性能，支持动态新增子元素的事件响应；
- 实现：给ul绑定click事件，判断`e.target.tagName === 'LI'`后执行逻辑。

### 3. load与DOMContentLoaded区别+绑定对象
- 区别：`DOMContentLoaded`**DOM树解析完成**就触发（不等待样式、图片加载）；`load`**页面所有资源（DOM、样式、图片、音频）加载完成**才触发；
- 绑定对象：`DOMContentLoaded`绑定在`document`上；`load`绑定在`window`上。

### 4. offsetWidth/clientWidth/scrollWidth核心区别
- `offsetWidth`：元素**可视宽度**，包含**内容+内边距+边框**，无滚动条；
- `clientWidth`：元素**可容纳内容的宽度**，包含**内容+内边距**，不含边框、滚动条；
- `scrollWidth`：元素**实际内容总宽度**，包含**内容+内边距**，含超出可视区域的滚动内容，不含边框、滚动条。

### 5. 获取卷去顶部距离+平滑滚动到顶部
- 获取卷去顶部：`document.documentElement.scrollTop || document.body.scrollTop`（兼容各浏览器）；
- 平滑滚动到顶：`window.scrollTo({ top: 0, behavior: 'smooth' })`。

### 6. 电梯导航核心逻辑+模块视口判断
- 核心逻辑：① 页面滚动时，判断各模块是否进入视口，同步激活对应导航项；② 点击导航项，平滑滚动到对应模块位置；
- 视口判断：获取模块的`offsetTop`和页面卷去高度，满足`卷去高度 ≥ 模块offsetTop - 导航高度`即进入视口。

### 7. 全选反选-全选框与小复选框状态同步
- **全选框控制小复选框**：全选框勾选/取消时，遍历所有小复选框，将其`checked`设为与全选框一致；
- **小复选框控制全选框**：每个小复选框点击时，判断**所有小复选框是否都被勾选**，结果赋值给全选框的`checked`。
