...
### 1. getMonth()/getDay()取值范围+转换实际月份/星期
- 取值：`getMonth()` 0-11（对应1-12月）；`getDay()` 0-6（对应周日-周六）。
- 转换：月份+1即可；星期可配数组`['日','一','二','三','四','五','六']`，用`getDay()`结果做索引取值。

### 2. 获取时间戳三种方式+特点
1. `new Date().getTime()`：兼容性最好，适用于所有浏览器；
2. `+new Date()`：简洁写法，通过隐式转换获取，效果同getTime()；
3. `Date.now()`：ES6新增，写法最简洁，不支持低版本IE浏览器。

### 3. children和childNodes区别+推荐使用
- 区别：`children` 仅返回**元素子节点**（DOM元素）；`childNodes` 返回**所有子节点**（含元素、文本、注释节点）。
- 推荐：`children`，开发中仅需操作元素节点，无需处理无用的文本/注释节点。

### 4. 动态创建元素三步流程+避免频繁回流
- 三步：① `document.createElement()` 创建元素；② 给元素设置属性/内容；③ `appendChild()`/`insertBefore()` 添加到DOM。
- 避免回流：创建文档碎片`document.createDocumentFragment()`，先将所有新元素添加到碎片，最后一次性把碎片插入DOM。

### 5. 移动端常用触摸事件+触发场景
- `touchstart`：手指**触摸到屏幕**时触发；
- `touchmove`：手指**在屏幕上滑动**时持续触发；
- `touchend`：手指**离开屏幕**时触发；
- `touchcancel`：触摸被意外中断（如弹窗、滑动刷新）时触发。

### 6. 回流和重绘区别+触发操作+优化
- 区别：**重绘**仅改变元素样式（如颜色、背景），不改变布局，性能损耗小；**回流**改变元素布局/尺寸，会触发页面重新计算布局，性能损耗大，回流必引发重绘。
- 触发回流：修改元素宽高/边距、增删DOM节点、滚动页面、改变窗口大小。
- 优化：① 批量修改样式（用class/碎片）；② 隐藏元素后修改样式；③ 避免频繁获取offset/scroll系列属性。

### 7. 学生信息表优先操作数据而非直接操作DOM的原因
- DOM操作是**浏览器重排重绘**的核心触发源，频繁操作会大幅降低页面性能，出现卡顿；
- 先操作JS数据（如数组），仅在数据更新完成后，一次性渲染/更新DOM，减少DOM操作次数，提升页面性能；同时数据与视图分离，便于后续数据的查询、修改、筛选等逻辑处理。
