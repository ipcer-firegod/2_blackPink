...
### 1. 事件监听三要素；DOM0/DOM2 核心区别
- 三要素：**事件源**（触发事件的DOM元素）、**事件类型**（如click/mouseover）、**事件处理函数**（触发后执行的函数）。
- 核心区别：DOM0（`元素.onclick = 函数`）**一个事件只能绑定一个处理函数**，新函数会覆盖旧函数；DOM2（`元素.addEventListener(类型, 函数)`）**一个事件可绑定多个处理函数**，按绑定顺序执行，还支持捕获/冒泡阶段选择、移除指定处理函数。

### 2. 常用鼠标/焦点/键盘事件+触发场景
- **鼠标事件**：`click`（单击元素）、`mouseover`（鼠标移入元素，含子元素）、`mouseout`（鼠标移出元素，含子元素）、`mouseenter`（鼠标移入元素，不含子元素）、`mouseleave`（鼠标移出元素，不含子元素）、`mousemove`（鼠标在元素内移动）。
- **焦点事件**：`focus`（元素获取焦点，不冒泡）、`blur`（元素失去焦点，不冒泡）、`focusin`（获取焦点，冒泡）、`focusout`（失去焦点，冒泡）。
- **键盘事件**：`keydown`（键盘按键按下，所有键都触发）、`keyup`（键盘按键松开）、`keypress`（按键按下，仅字符键触发，已逐步废弃）。

### 3. 获取事件对象；clientX/offsetX 区别
- 获取事件对象：在事件处理函数中定义形参（如`e/ev/event`），`元素.onclick = (e) => { /* e 就是事件对象 */ }`/`addEventListener('click', (e) => {})`，浏览器会自动传入。
- 区别：`clientX` 是鼠标相对于**浏览器可视区域左上角**的X坐标；`offsetX` 是鼠标相对于**触发事件的元素左上角**的X坐标。

### 4. 事件处理函数/定时器回调中 this 指向
- 事件处理函数（DOM0/DOM2 普通函数）：`this` 指向**事件源**（触发事件的DOM元素）；（箭头函数则指向外部作用域的this）。
- 定时器回调函数（`setInterval/setTimeout`）：默认`this` 指向**window**（箭头函数则指向定义时的外部作用域this）。

### 5. 回调函数；实际开发场景
- 定义：**把一个函数作为参数传入另一个函数**，在另一个函数内部满足条件时执行，是异步编程的基础。
- 场景1：事件监听回调（`btn.addEventListener('click', fn)`，点击时执行fn）；场景2：定时器回调（`setTimeout(fn, 1000)`，1秒后执行fn）；场景3：AJAX请求回调（请求成功后执行的处理函数）。

### 6. 阻止表单默认提交；阻止事件冒泡
- 阻止表单默认提交：在表单`submit`事件中，调用事件对象的`e.preventDefault()`；也可在提交按钮`click`中返回`false`（DOM0方式）。
- 阻止事件冒泡：调用事件对象的`e.stopPropagation()`；若需同时阻止冒泡+默认行为，可写`e.stopPropagation(); e.preventDefault()`。

### 7. 鼠标经过选项卡切换内容核心逻辑
1. 获取得所有选项卡标签元素和对应的内容元素；
2. 给每个标签绑定`mouseover`事件，传入**索引**标识当前标签；
3. 事件触发时，先**移除所有标签的激活类**、**隐藏所有内容元素**；
4. 给**当前索引的标签**添加激活类，**显示当前索引的内容元素**。
（核心：标签与内容**按索引一一对应**，排他思想实现唯一激活）。
